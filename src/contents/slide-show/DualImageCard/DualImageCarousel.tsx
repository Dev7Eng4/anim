...
import { z } from "zod";
import { DualImageCard, DualImageDataSchema } from "./DualImageCard";
...
export const DualImageCarouselSchema = z.object({
  ...
  durationPerCard: z.number().optional().default(6),
...
export const DualImageCarousel: React.FC<DualImageCarouselProps> = ({
  ...
  // Animation timing — driven by durationPerCard prop
  // Animation timing — fixed 2s for the reveal animation to match SingleImageCard
  const INTRO_DURATION_PER_CARD = 2 * fps;
  ...
  // Scrolling phase settings (scroll speed matches durationPerCard)
