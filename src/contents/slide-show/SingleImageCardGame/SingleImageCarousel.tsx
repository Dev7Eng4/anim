...
import {
  SingleImageCard,
  SingleImageDataSchema,
  CardVariantEnum,
} from "./SingleImageCard";
...
export const SingleImageCarouselSchema = z.object({
  ...
  background: z.string(),
  /** 'slide' = dạng 1 (mặc định), 'flip' = dạng 2 (lật 180°), 'zoom' = dạng 3 (scale) */
  imageAnimation: z.enum(["slide", "flip", "zoom"]).optional(),
  /**
   * Loại thẻ mặc định cho toàn carousel.
   * Per-item `cardVariant` trong data sẽ override giá trị này.
   */
...
export const SingleImageCarousel: React.FC<SingleImageCarouselProps> = ({
  ...
  const GAP = 60; // Larger gap for rarity glow separation
      ...
      >
        {items.map((item, index) => {
          ...
          const cardProps = {
            ...
            data: item,
            // cardVariant is already in restProps (from schema spread), and per-item variant in data.cardVariant overrides it inside SingleImageCard
          };
