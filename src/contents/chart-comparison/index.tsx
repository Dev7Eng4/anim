...
export const ChartComparisonSchema = z.object({
  title: z.string(),
  // Thời gian tổng cộng để animate (tính bằng giây)
  animationDuration: z.number(),
  // Giới hạn số lượng item hiển thị cùng lúc
  maxItems: z.number(),
  // Khoảng cách giữa các bar
  barSpacing: z.number(),
  // Khoảng thời gian (tháng) để một item đạt được gross tối đa
  growthMonths: z.number(),
  // Chiều cao của mỗi bar
  barItemHeight: z.number(),
  // Thời gian (giây) cho mỗi bước nhảy tháng
  jumpDuration: z.number(),
  ...
  items: z.array(
    z.object({
      title: z.string(),
      startTime: z.string(), // Định dạng MM/YYYY
      gross: z.number(),
...
export const ChartComparison: React.FC<ChartComparisonProps> = (props) => {
  ...
  const barValueGap = 15; // Khoảng cách từ bar đến giá trị
